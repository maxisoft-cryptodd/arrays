...
template <typename T>
cryptodd::memory::vector<T> generate_temporal_2d_data(size_t rows, size_t cols) {
    cryptodd::memory::vector<T> data(rows * cols);
    if constexpr (std::is_floating_point_v<T>) {
        cryptodd::memory::vector<T> prev_row(cols);
        for (size_t j = 0; j < cols; ++j) {
            prev_row[j] = static_cast<T>(100.0 + j * 10.0);
        }
        for (size_t i = 0; i < rows; ++i) {
            for (size_t j = 0; j < cols; ++j) {
                T change = static_cast<T>((i + j) % 7 - 3) * 0.01;
                data[i * cols + j] = prev_row[j] + change;
                prev_row[j] = data[i * cols + j];
            }
        }
    } else if constexpr (std::is_integral_v<T>) {
        cryptodd::memory::vector<T> prev_row(cols);
        for (size_t j = 0; j < cols; ++j) {
            prev_row[j] = 1678912345678901 + j * 1000;
        }
        for (size_t i = 0; i < rows; ++i) {
            for (size_t j = 0; j < cols; ++j) {
                T change = (i + j) % 10 + 1;
...
class CApiTemporal2dSimdTest : public ::testing::TestWithParam<TestConfig2D> {
protected:
    ...
    void run_workflow() {
        ...
        ASSERT_EQ(config.shape.size(), 2);
        const size_t total_rows = config.shape[0];
        const size_t num_cols = config.shape[1];

        auto full_data = test_helpers::generate_temporal_2d_data<T>(total_rows, num_cols);

        size_t rows1 = total_rows;
        size_t rows2 = 0;
        ...
        if (config.use_append) {
            rows1 = total_rows / 2;
            rows2 = total_rows - rows1;
            size_t split_point = rows1 * num_cols;
        ...
        // --- READ PHASE ---
        {
            ...
            if constexpr (std::is_floating_point_v<T>) {
                float tolerance = is_f16_codec ? 0.005f : 1e-6f;
...
TEST_P(CApiTemporal2dSimdTest, FullWorkflow) {
    const auto& config = GetParam();
...
INSTANTIATE_TEST_SUITE_P(
    Temporal2dSimdCases,
    CApiTemporal2dSimdTest,
    ::testing::Values(
        TestConfig2D{"F32", "TEMPORAL_2D_SIMD_F32", "FLOAT32", {1000, 10}, false},
        TestConfig2D{"F16", "TEMPORAL_2D_SIMD_F16_AS_F32", "FLOAT32", {1000, 10}, false},
        TestConfig2D{"I64", "TEMPORAL_2D_SIMD_I64", "INT64", {800, 12}, false},
        TestConfig2D{"Append_F32", "TEMPORAL_2D_SIMD_F32", "FLOAT32", {2000, 8}, true},
        TestConfig2D{"Append_I64", "TEMPORAL_2D_SIMD_I64", "INT64", {1500, 15}, true}
    ),
    [](const ::testing::TestParamInfo<CApiTemporal2dSimdTest::ParamType>& info) {
